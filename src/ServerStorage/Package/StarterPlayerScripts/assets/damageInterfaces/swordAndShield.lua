...
if cab then for _db,adb in pairs(cab)do if
adb.id==dcb and adb.rank>0 then return true end end end;return false end;local cbb=false
local function dbb()if cbb then return end;cbb=true
while cbb do
if


dda.swordAndShieldAnimations.strike1.IsPlaying or dda.swordAndShieldAnimations.strike2.IsPlaying or dda.swordAndShieldAnimations.strike3.IsPlaying then if c_b then
...
function _ba:attack()
if not dda or not dda.swordAndShieldAnimations then return elseif _bb then return elseif
not aab then return elseif
not dab.Character or not dab.Character.PrimaryPart or
dab.Character.PrimaryPart.state.Value=="dead"then return elseif
dda.swordAndShieldAnimations.strike1.IsPlaying and(not b_b or not d_b)then return elseif
dda.swordAndShieldAnimations.strike2.IsPlaying and not a_b then return elseif
dda.swordAndShieldAnimations.strike3.IsPlaying then return end;local dcb=bab.entity.AnimationController
for _db,adb in
pairs(dcb:GetPlayingAnimationTracks())do if
adb.Name=="rock_throw_upper"or adb.Name=="rock_throw_upper_loop"then return end end
if
dda.swordAndShieldAnimations.strike1.IsPlaying and b_b then if __b then __b:disconnect()__b=nil end
if slashAnimationKeyframeConnection then
slashAnimationKeyframeConnection:disconnect()slashAnimationKeyframeConnection=nil end
dda.swordAndShieldAnimations.strike1:Stop()
__b=dda.swordAndShieldAnimations.strike2.Stopped:connect(_cb)
slashAnimationKeyframeConnection=dda.swordAndShieldAnimations.strike2.KeyframeReached:connect(acb)
ada:replicateClientAnimationSequence("swordAndShieldAnimations","strike2")_da=bba:GenerateGUID(false)spawn(dbb)elseif
not
dda.swordAndShieldAnimations.strike1.IsPlaying and(not
dda.swordAndShieldAnimations.strike2.IsPlaying or a_b)then
if


_ab and dda.swordAndShieldAnimations.strike2.IsPlaying and dda.swordAndShieldAnimations.strike2.TimePosition>=
dda.swordAndShieldAnimations.strike2.Length*0.3 and dda.swordAndShieldAnimations.strike2.TimePosition<=
dda.swordAndShieldAnimations.strike2.Length*0.7 then if __b then __b:disconnect()__b=nil end
if slashAnimationKeyframeConnection then
slashAnimationKeyframeConnection:disconnect()slashAnimationKeyframeConnection=nil end
dda.swordAndShieldAnimations.strike2:Stop()
__b=dda.swordAndShieldAnimations.strike3.Stopped:connect(_cb)
slashAnimationKeyframeConnection=dda.swordAndShieldAnimations.strike3.KeyframeReached:connect(acb)
ada:replicateClientAnimationSequence("swordAndShieldAnimations","strike3")_da=bba:GenerateGUID(false)spawn(dbb)else if __b then
__b:disconnect()__b=nil end
if slashAnimationKeyframeConnection then
slashAnimationKeyframeConnection:disconnect()slashAnimationKeyframeConnection=nil end
dda.swordAndShieldAnimations.strike2:Stop()
__b=dda.swordAndShieldAnimations.strike1.Stopped:connect(function()
c_b=false;a_b=false;b_b=false end)
slashAnimationKeyframeConnection=dda.swordAndShieldAnimations.strike1.KeyframeReached:connect(acb)
ada:replicateClientAnimationSequence("swordAndShieldAnimations","strike1")_da=bba:GenerateGUID(false)spawn(dbb)end end end
...
dda=ada:getAnimationsForAnimationController(bab.entity.AnimationController)end end;function _ba:unequip()end;local function bcb(dcb,_db)
if dcb=="abilities"then cab=_db;d_b=bbb(3)_ab=bbb(30)end end
local function ccb()
