...
script.Parent.header.username.Size=UDim2.new(0,a_a+10,0,26 +10)end;ac()
repeat wait()until game.Players.LocalPlayer.Character;local bc=game.Players.LocalPlayer.Character
while

not bc.PrimaryPart and bc.Parent and bc:IsDescendantOf(workspace)do local dd=bc:WaitForChild("hitbox",1)if dd then bc.PrimaryPart=dd;break else
warn("Waiting for PrimaryPart in",script.Name)end end;aa=bc.PrimaryPart;aa:WaitForChild("health")
...
aa.maxMana.Changed:connect(dc)dc()local _d=da.levels;local ad=da.network
local function bd()
...
math.floor(d_a).."/".._aa
local aaa=d_a-bb
local baa=script.parent.content.noticeTemplate:Clone()baa.Name="Notice"baa.TextTransparency=1;baa.TextStrokeTransparency=1
baa.Parent=script.Parent.content;baa.Visible=true
baa.Text="+"..math.floor(aaa).." EXP"
baa.Position=UDim2.new(1,ca:NextInteger(-10,50),0,ca:NextInteger(30,70))
da.tween(baa,{"Position"},baa.Position+UDim2.new(0,0,0,-100),3)
da.tween(baa,{"TextTransparency","TextStrokeTransparency"},{0,0.7},1.5)
local caa=script.Parent.header.xp.value.tip.sample
for i=1,6 do local daa=caa:Clone()daa.Rotation=math.random(1,90)
daa.Parent=caa.Parent;daa.Visible=true
_c(daa,{"Rotation","Position","Size","BackgroundTransparency"},{
daa.Rotation+math.random(100,200),UDim2.new(0,math.random(3,25),0.5,math.random(-20,20)),UDim2.new(0,16,0,16),1},
math.random(60,130)/100)game.Debris:AddItem(daa,1.5)end
if d_a<bb then
...
script.Parent.header.xp.value.Size=UDim2.new(1,0,0,0)local daa=UDim2.new(d_a/_aa,0,1,0)
da.tween(script.Parent.header.xp.value,{"Size"},{daa},0.5)
script.Parent.header.xp.instant.Size=daa end)else
da.tween(script.Parent.header.xp.value,{"Size"},{UDim2.new(d_a/_aa,0,1,0)},1)
script.Parent.header.xp.instant.Size=UDim2.new(d_a/_aa,0,1,0)end;bb=__a
spawn(function()wait(0.5)
da.tween(baa,{"TextTransparency","TextStrokeTransparency"},{1,1},1.5)wait(3)if bb==__a and not ba then end end)end end
...
ad:connect("{B5BFDE4F-1C2E-4D65-ACB4-277DB309BF6D}","Event",cd)bd()end;return _a