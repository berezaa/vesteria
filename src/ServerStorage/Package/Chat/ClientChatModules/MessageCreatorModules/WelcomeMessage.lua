...
function CreateWelcomeMessageLabel(da,_b)
if _b~=da.OriginalChannel then if aa.ShowChannelsBar then return end end;local ab=da.Message;local bb=da.ExtraData or{}
local cb=bb.Font or aa.DefaultFont;local db=bb.FontSize or aa.ChatWindowTextSize;local _c=bb.ChatColor or
aa.DefaultMessageColor
local ac=bb.ChannelColor or _c;local bc,cc=ca:CreateBaseMessage(ab,cb,db,_c)local dc=nil
if
_b~=da.OriginalChannel then local __a=string.format("{%s} ",da.OriginalChannel)
...
{[ca.KEY_MESSAGE_TYPE]=ba.MessageTypeWelcome,[ca.KEY_CREATOR_FUNCTION]=CreateWelcomeMessageLabel}