...
local _d={id=62,name="TACTICAL NUKE INCOMING",image="rbxassetid://1372924008",description="DOO-WEEP! DOO-WEEP! DOO-WEEP!",layoutOrder=0,maxRank=1,statistics={[1]={cooldown=1,range=256,damageMultiplier=9001,percentHealthBurned=100000,radius=256,manaCost=0}},windupTime=2,securityData={playerHitMaxPerTag=64,isDamageContained=true,projectileOrigin="character"},targetingData={targetingType="directSphere",range="range",radius="radius",onStarted=function(ad,bd)
...
db:invoke("{27791132-7095-4A6F-B12B-2480E95768A2}",true)end
local baa=script.cast:Clone()baa.Parent=__a;baa:Play()
bb:AddItem(baa,baa.TimeLength)wait(self.windupTime)if cd then
db:invoke("{27791132-7095-4A6F-B12B-2480E95768A2}",false)end;aaa:Stop(0.25)d_a.Enabled=false
local caa=bd["ability-statistics"]["radius"]local daa=bd["ability-statistics"]["range"]
local _ba=bd["absolute-mouse-world-position"]local aba=_ba-c_a.WorldPosition;if aba.magnitude>=daa then
aba=aba.unit*daa end;_ba=c_a.WorldPosition+aba;local bba=1.25
local function cba()
