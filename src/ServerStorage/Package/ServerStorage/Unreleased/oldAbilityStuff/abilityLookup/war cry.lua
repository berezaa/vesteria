...
local cd={id=47,name="War Cry",image="rbxassetid://4079576521",description="Grant an offense buff to yourself and nearby party members.",mastery="Bigger buff.",layoutOrder=0,maxRank=10,statistics=ad.calculateStats{maxRank=10,static={cooldown=
60 *5,radius=32},staggered={duration={first=60 *2.5,final=60 *5,levels={2,3,5,6,8,9}},buff={first=5,final=15,levels={4,7,10}}},pattern={manaCost={base=50,pattern={2.5,2.5,5}}}},windupTime=0.6,securityData={}}
...
ac:invoke("{964385B0-1F06-4732-A494-F5D6F84ABC61}",aba,"empower",{duration=bba.duration,modifierData={damageGivenMulti=
...
for c_a,d_a in pairs(a_a)do local _aa=script.sword:Clone()
...
local _aa=__a.entity.AnimationController:LoadAnimation(cb["shout"])_aa:Play()
if b_a then
...
db:AddItem(aaa,aaa.TimeLength)wait(self.windupTime)local baa=script.hit:Clone()
baa.Parent=d_a;baa:Play()db:AddItem(baa,baa.TimeLength)
local caa=a_a["ability-statistics"]["radius"]local daa=caa*2
do local _ba=0.25;local aba=1;local bba=script.ring:Clone()
bba.Position=d_a.Position;bba.Parent=bd
dc(bba,{"Size"},Vector3.new(daa,2,daa),_ba)dc(bba,{"Transparency"},1,aba)
db:AddItem(bba,aba)end;if b_a then
