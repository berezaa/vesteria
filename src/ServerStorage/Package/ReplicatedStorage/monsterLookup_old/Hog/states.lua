...
bc.__directRoamTheta=ad;bc.__blockConfidence=0;bc.__LAST_ROAM_TIME=tick()return"direct-roam"end;bc.__LAST_ROAM_TIME=tick()-4.5;return end end else return"sleeping"end end,execute=function(bc,cc,dc,_d)if

_d:FindFirstChild("entity")and _d.entity.PrimaryPart:FindFirstChild("attacking")then
_d.entity.PrimaryPart.running:Stop()end end},["wait-roaming"]={animationEquivalent="idling",transitionLevel=3,step=function(bc,cc)
if
...
if
bc.__PATHFIND_QUEUE_TIME and tick()-bc.__PATHFIND_QUEUE_TIME>5 then bc:resetPathfinding()return"idling"end end},["aggro"]={animationEquivalent="precharge",transitionLevel=3,lockTimeForPreventStateTransition=1.8,step=function(bc,cc)
local dc=bc.manifest.Position;local _d=bc.closestEntity;local ad=bc.targetEntity
if ad then local bd=ad.Position
...
bc.manifest.BodyGyro.CFrame=CFrame.new(dc,Vector3.new(b_a.X,cd.Y,b_a.Z))return"running"else return"idling"end end,execute=function(bc,cc,dc,_d)if
_d and _d.Parent and _d:FindFirstChild("entity")then
_d.entity.HumanoidRootPart.charge:Play()end end},["direct-roam"]={animationEquivalent="walking",transitionLevel=3,step=function(bc,cc)
if
...
bc.manifest.BodyVelocity.Velocity=Vector3.new()bc:resetPathfinding()return"idling"end end end},["following"]={animationEquivalent="walking",transitionLevel=4,execute=function(bc,cc,dc,_d)if
_d and _d.Parent and _d:FindFirstChild("entity")then
...
if bb.magnitude(cd-dc)<=bc.attackRange then return"attack-ready"end end},["running"]={animationEquivalent="charge",transitionLevel=4,step=function(bc,cc)if
not bc.targetEntity then return"idling"end
local dc=bc.manifest.Position;local _d=bc.targetEntity;local ad=bc.targetEntity;local bd=ad.Position;local cd=bd
if
bc.manifest.BodyVelocity.MaxForce.Y<=0.1 then cd=Vector3.new(bd.X,dc.Y,bd.Z)end
...
if
bc:isTargetEntityInLineOfSight(nil,true)then
bc.manifest.BodyVelocity.Velocity=__a*bc.baseSpeed*4;bc.manifest.BodyGyro.CFrame=CFrame.new(dc,cd)
bc.__LAST_POSITION_SEEN=a_a;bc.__LAST_MOVE_DIRECTION=__a*bc.baseSpeed*4 else
if
not bc.__LAST_POSITION_SEEN and false then bc:setTargetEntity(nil,nil)
bc.manifest.BodyVelocity.Velocity=Vector3.new()return"idling"elseif bc.__LAST_POSITION_SEEN then
local b_a=Vector3.new(bc.__LAST_POSITION_SEEN.X,dc.Y,bc.__LAST_POSITION_SEEN.Z)
bc.manifest.BodyVelocity.Velocity=(b_a-dc).unit*bc.baseSpeed*4
...
bc.manifest.BodyVelocity.Velocity=Vector3.new()end else return"running"end end},["dive"]={transitionLevel=7,step=function(bc,cc)
bc.manifest.BodyVelocity.Velocity=Vector3.new()return"on-ground"end},["on-ground"]={transitionLevel=7,lockTimeForLowerTransition=1.5,step=function(bc,cc)
bc.manifest.BodyVelocity.Velocity=Vector3.new()
...
bc.manifest.BodyGyro.CFrame=CFrame.new(_d,Vector3.new(dd.X,_d.Y,dd.Z))if dc<=bc.aggressionRange/2 then return"running"end
return"aggro"end end;return"idling"end},["attacking"]={transitionLevel=6,animationEquivalent="gallop",animationPriority=Enum.AnimationPriority.Action,doNotLoopAnimation=true,doNotStopAnimation=true,execute=function(bc,cc,dc,_d)
if
...
_d.entity.PrimaryPart.attacking:Play()end
local dd=cc.Length* (dc.animationDamageStart or 0)wait(dd)
local __a=cc.Length* (dc.animationDamageEnd or 0.3)local a_a=__a-dd;local b_a=a_a/10
for i=0,a_a,b_a do
...
db:fire("{7EEEF31B-024D-429C-8220-4A6094CFA5B8}",(baa*50 +Vector3.new(0,40,0)))
_d.clientHitboxToServerHitboxReference.Value.BodyVelocity.Velocity=Vector3.new()end elseif d_a.castType=="box"then
...
local baa=( (bd.Position-cd.Position)*Vector3.new(1,0,1)).unit
db:fire("{7EEEF31B-024D-429C-8220-4A6094CFA5B8}",(baa*50 +Vector3.new(0,40,0)))
_d.clientHitboxToServerHitboxReference.Value.BodyVelocity.Velocity=Vector3.new()end end end end;wait(b_a)else break end end end,verify=function(bc)if
