...
function abilityData:execute(renderCharacterContainer, abilityExecutionData, isAbilitySource, guid)
	...
	if abilityExecutionData["ability-state"] == "begin" then
		...
		if isAbilitySource then
			local newAbilityExecutionData 				= network:invoke("getAbilityExecutionData", abilityData.id)
			...
			network:fireServer("replicateAbilityStateChangeByPlayer", abilityData.id, "update", newAbilityExecutionData, guid)
		end
	elseif abilityExecutionData["ability-state"] == "update" then
		...
		if isAbilitySource then
			...
			network:fireServer("replicateAbilityStateChangeByPlayer", abilityData.id, "end")
		end
