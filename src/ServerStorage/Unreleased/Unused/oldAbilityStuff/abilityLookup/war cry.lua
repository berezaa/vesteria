...
local abilityData = {
	...
	description = "Grant an offense buff to yourself and nearby party members.",
	mastery = "Bigger buff.",
	layoutOrder = 0;
	maxRank = 10,
	...
	local function buffTarget(target)
		...
		network:invoke(
			...
			"empower",
			{
				...
				modifierData = {
					damageGivenMulti = stats.buff / 100,
				},
			},
			manifest,
			"ability",
...
function abilityData:execute_client(abilityExecutionData, affected)
	...
	for _, target in pairs(affected) do
		...
		local size = sword.Size
		sword.Size = Vector3.new()
		tween(sword, {"Size"}, size, 0.5)
...
function abilityData:execute(renderCharacterContainer, abilityExecutionData, isAbilitySource, guid)
	...
	local track = renderCharacterContainer.entity.AnimationController:LoadAnimation(abilityAnimations["shout"])
	track:Play()
