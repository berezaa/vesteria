...
local function doesInventoryHaveSpace(player, category)
...
local function prepareManifest(physItem)
	...
	if physItem:IsA("BasePart") then
		...
		for i, Child in pairs(physItem:GetChildren()) do
...
local function generateItemManifest(itemDropData, physItem)
	...
	if physItem then
		...
		if itemBaseData.equipmentType == "arrow" then
			...
			if itemDropData.stacks == 1 then
				physItem 				= itemBaseData.module.manifest:Clone()
				physItem.CanCollide 	= false
				physItem.Anchored 		= false
			else
				-- todo: how to do multiple quivers?
				physItem 			= game.ReplicatedStorage.entities.ArrowUpperTorso2.quiver:Clone()
				physItem.Anchored 	= false
				physItem.CanCollide = false
				...
				for ai = 1, arrowParts do
					local arrow 		= itemBaseData.module.manifest:Clone()
		...
		elseif itemBaseData.module:FindFirstChild("manifest", true) then
			physItem = itemBaseData.module:FindFirstChild("manifest", true):Clone()
			physItem = prepareManifest(physItem)
		elseif itemBaseData.module:FindFirstChild("container") then
			local primaryPart = itemBaseData.module.container.PrimaryPart
			
			physItem = primaryPart:Clone()
			physItem:ClearAllChildren()
			
			for ii, vv in pairs(primaryPart:GetChildren()) do
			...
			for i, v in pairs(itemBaseData.module.container:GetChildren()) do
	...
	local itemMask 	= replicatedStorage.entities.itemMask:Clone()
	...
	if dye then
		...
		for i,v in pairs(physItem:GetDescendants()) do
	...
	if not itemDropData.isNotDropping then
		...
		if (itemInfo.rarity and itemInfo.rarity == "Rare") or (itemInfo.category and itemInfo.category == "equipment") then
			...
			for i,child in pairs(script.rareItem:GetChildren()) do
...
local function spawnItemOnGround(lootDrop, dropPosition, owners, physItem)
	if itemLookup[lootDrop.id] then
		...
		-- ber edit: removed this bc it caused issues with ownerless items
		--[[
		else
			return
...
local numberItemDropGenerator = Random.new()
local function onMonsterDeathDropLoot(monster)
	if not monster then return end
	
	local currentMonsterData = monsterLookup[monster.Name]
	if currentMonsterData then
		currentMonsterData = require(currentMonsterData)
		
		for i, lootDrop in pairs(currentMonsterData.lootDrops) do
			if lootDrop.spawnChance > numberItemDropGenerator:NextNumber() then
				if itemLookup[lootDrop.id] then					
					spawnItemOnGround(lootDrop, monster.Position)
				end
			end
		end
	end
...
local function onActivateItemRequestReceived(player, category, inventorySlotPosition, _itemId, playerInput)
	local playerData = network:invoke("getPlayerData", player)
	local inventorySlotData = network:invoke("getPlayerInventorySlotDataByInventorySlotPosition", player, category, inventorySlotPosition)
	if playerData and inventorySlotData and (not _itemId or inventorySlotData.id == _itemId) then
		...
		if itemBaseData.category == "consumable" or itemBaseData.activationEffect ~= nil then
			...
			if tick() - (playerConsumeCooldownTable[player] or 0) >= CONSUMABLE_COOLDOWN_TIME * math.clamp(1 - playerData.nonSerializeData.statistics_final.consumeTimeReduction, 0, 1) then
				...
				if worked then
					...
					if playerData.nonSerializeData.statistics_final.itemUseGain10PercentDEF then
						local wasApplied, reason = network:invoke("applyStatusEffectToEntityManifest", player.Character.PrimaryPart, "empower", {
							duration = 5;
							
							modifierData = {
								defense = playerData.nonSerializeData.statistics_final.equipmentDefense * 0.1;
							};
						}, player.Character.PrimaryPart, "perk", 0)
					end
					
					--local itemWasRemoved = network:invoke("removePlayerInventorySlotData", player, category, inventorySlotData.position, itemBaseData.id, 1)
					local source = "item:"..itemBaseData.module.Name
					-- 																(player, inventoryTransferDataCollection_player, gold_player, inventoryTransferData_intermediateCollection_NPC, gold_NPC)
					
					if itemBaseData.category ~= "miscellaneous" then
						local success = network:invoke("tradeItemsBetweenPlayerAndNPC",  player, {{id = inventorySlotData.id; position = inventorySlotData.position; stacks = 1}}, 0, {}, 0, source)
					end
					
					
				else
					network:fireClient("signal_alertChatMessage", player, {Text = "Failed to use item: "..status or "no error."; Font = Enum.Font.SourceSans; Color = Color3.fromRGB(216, 161, 107)} )		
		...
		elseif itemBaseData.category == "equipment" then
...
local function onPlayerRequest_buyItemFromShop(player, inventorySlotData, stacksBeingRequested, inventoryModule)
	...
	local shopkeeperInventory = require(inventoryModule) do
		...
		for i, shopkeeperItemInfo in pairs(shopkeeperInventory) do
	...
	if player and inventorySlotData and typeof(inventorySlotData) == "table" and stacksBeingRequested and type(stacksBeingRequested) == "number" and stacksBeingRequested >= 1 and stacksBeingRequested == clamp_stacksBeingRequested then
		...
		if confirmedItemInfo and confirmedCostInfo and confirmedCostInfo.costType and itemCost then
			if confirmedCostInfo.costType == "item" then
				...
				success, reason = network:invoke("tradeItemsBetweenPlayerAndNPC",  player, {{id = confirmedCostInfo.costId; stacks = itemCost * stacksBeingRequested}}, 0, {itemBeingBought}, 0, source)
...
--			end
			...
			success, reason 		= network:invoke("tradeItemsBetweenPlayerAndNPC",  player, {}, math.clamp((itemBaseData.buyValue or 1) * coinCostReduction, 1, math.huge) * stacksBeingRequested, {itemBeingBought}, 0, source)
		end
		--[[
		if success then
			network:fire("questTriggerOccurred", player, "item-collected", {id = inventorySlotData.id; amount = stacksBeingRequested})
		end
		]]
...
local function onPlayerRequest_sellItem(player, unsafeInventorySlotData, stacksToRemove)
	...
	if inventorySlotData and typeof(inventorySlotData) == "table" and stacksToRemove == stacksToRemove and stacksToRemove and type(stacksToRemove) == "number" then
		...
		local success = network:invoke("tradeItemsBetweenPlayerAndNPC",  player, {{id = inventorySlotData.id; position = inventorySlotData.position; stacks = stacksToRemove}}, 0, {}, (sellValue) * stacksToRemove, source)
...
local function applyPotionStatusEffectToEntityManifest_server(entityManifest, healthToRestore, manaToRestore, sourceType, sourceId)
	...
	if associatePlayer then
		local playerData = network:invoke("getPlayerData", associatePlayer)
		
		if playerData then
			healthToRestore = (healthToRestore or 0) 	* (1 + playerData.nonSerializeData.statistics_final.consumableHealthIncrease)
			manaToRestore 	= (manaToRestore or 0) 		* (1 + playerData.nonSerializeData.statistics_final.consumableManaIncrease)
		end
	end
	
	local wasApplied, reason = network:invoke("applyStatusEffectToEntityManifest", entityManifest, "potion", {
		duration = 3;
		
		healthToHeal 	= healthToRestore or 0;
		manaToRestore 	= manaToRestore or 0;
	}, entityManifest, sourceType, sourceId)
