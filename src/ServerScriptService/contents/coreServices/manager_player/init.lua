...
-- nil 		= not assigned
local function getPlayerQuestState(player, questId)
	local playerData = playerDataContainer[player]
	if playerData then
		...
		for i, completePlayerQuestData in pairs(playerData.quests.completed) do
...
local function getPlayerQuestCurrentObjective(player, questId)
	local playerData = playerDataContainer[player]
	if playerData then
		for i, playerQuestData in pairs(playerData.quests.active) do
			if playerQuestData.id == questId then
...
local function checkIfQuestIsCompleteByQuestId(player, questId, currentObjective)
	local playerData = playerDataContainer[player]
	if playerData then
		for i, playerQuestData in pairs(playerData.quests.active) do
			if playerQuestData.id == questId then
				local stepsComplete = 0
				local stepsTotal = 0
				
				--for ii, objectiveData in pairs(playerQuestData.objectives) do
					for iii, stepData in pairs(playerQuestData.objectives[currentObjective].steps) do
						stepsTotal = stepsTotal + 1 
						if stepData.requirement.amount <= stepData.completion.amount then
							stepsComplete = stepsComplete + 1 -- shouldn't have been possible to finish steps locked behind the next objective
						end
					end
					
				if #playerQuestData.objectives == currentObjective then
					return stepsTotal > 0 and stepsTotal == stepsComplete
				else
					if playerQuestData.objectives[currentObjective + 1].started then
						return false
					end
				end
...
local function checkForOutdatedQuests(player)
	local playerData = playerDataContainer[player]
	--local questLookup = 
	--local questInfo = questLookup[questData.id]
	for i, quest in pairs(playerData.quests.active) do
		-- first adding the system
		
		if quest.QUEST_VERSION == nil then
			if quest.id == 2 or quest.id == 1 then
				-- manual clear, these quests were changed
				table.remove(playerData.quests.active, i)
			else
				-- for players who had quests active before version implementation
				quest.QUEST_VERSION = questLookup[quest.id].QUEST_VERSION
			end
		elseif quest.QUEST_VERSION ~= questLookup[quest.id].QUEST_VERSION then
			table.remove(playerData.quests.active, i)
		elseif #quest.objectives ~= #questLookup[quest.id].objectives then
			table.remove(playerData.quests.active, i)
		else 
		end
...
--	network:fireClient("signal_teleport", destination)
	...
	if timestamp then
		...
		network:fireClient("alertPlayerNotification", player, {text = "Failed to save your data. Teleportation canceled."; textColor3 = Color3.fromRGB(255, 57, 60)})
		return false
	end	
end
...
local function createPartyTeleportData(playersToTeleport, destination, partyLeaderUserId, spawnLocation)
	...
	for i,player in pairs(playersToTeleport) do
		if player:FindFirstChild("teleporting") then
			network:fireClient("alertPlayerNotification", player, {text = "Party teleport failed: "..player.Name.." is already teleporting."; textColor3 = Color3.fromRGB(255, 57, 60)})
			return false
		end		
	end
	...
	for i,player in pairs(playersToTeleport) do
		...
		if playerTeleportData and playerTeleportData.partyData then
			...
			for e,oPlayer in pairs(playersToTeleport) do
				network:fireClient("alertPlayerNotification", oPlayer, {text = player.Name.." failed to teleport with the party."; textColor3 = Color3.fromRGB(234, 129, 59)})
			end
		end
	end
...
-- todo: generic implementation for if quest is done

local function isPlayerQuestActive(player, questId)
	local playerData = playerDataContainer[player]
	if playerData then
		for i, playerQuestData in pairs(playerData.quests.active) do
			if playerQuestData.id == questId then
...
--[[
	monster-killed triggerData {}
		string monsterName
	
	item-collected triggerData {}
		int id
		int amount[=1]
	
	item-lost triggerData {}
		int id
		int amount[=1]
--]]

-- todo: generalize trigger execution at some point?
local function onQuestTriggerOccured(player, triggerType, triggerData)
	-- need direct access to the playerData so we can modify it
	local playerData = playerDataContainer[player]
	
	if playerData then
		
		local questsModified = {}
		
		if triggerType == "monster-killed" then
			
			-- add kill to the bounty book
			
			if player:FindFirstChild("bountyHunter") then
			
				local monsterName = triggerData.monsterName
				playerData.bountyBook[monsterName] = (playerData.bountyBook[monsterName]) or {kills = 0; lastBounty = 0}	
				playerData.bountyBook[monsterName].kills = playerData.bountyBook[monsterName].kills + 1
				onClientRequestPropogateCacheData(player, "bountyBook")
			
			end

			for i, playerQuestData in pairs(playerData.quests.active) do
				-- update objective data that match this monster
				local modifiedQuest = false
				
				
				for ii, playerObjectiveData in pairs(playerQuestData.objectives) do
					if ii > playerQuestData.currentObjective then break end -- can't make progress on a future step
					if not playerObjectiveData.started then break end -- if you haven't talked through the next dialogue yet
					for iii, playerStepData in pairs(playerObjectiveData.steps) do
						if playerStepData.triggerType == triggerType and (not playerStepData.requirement.monsterName or playerStepData.requirement.monsterName == triggerData.monsterName) and playerStepData.completion.amount < playerStepData.requirement.amount then
							if playerStepData.requirement.isGiant and not (triggerData.monster.giant or triggerData.monster.superGiant or triggerData.monster.gigaGiant)  then
								break
							end
							
							playerStepData.completion.amount = playerStepData.completion.amount + 1
							if not modifiedQuest then
								table.insert(questsModified, playerQuestData)
								modifiedQuest = true
							end
							

							local monsterName = playerStepData.requirement.monsterName or "Any monster"
							if playerStepData.hideAlert == nil then
								network:fireClient("alertPlayerNotification", player, {id = playerQuestData.id .. "s" .. monsterName;
								 text = (monsterName) .. ": " .. math.clamp(playerStepData.completion.amount,0, playerStepData.requirement.amount) .. " / " .. playerStepData.requirement.amount})
							end
							
							if playerStepData.completion.amount >= playerStepData.requirement.amount then
								local questData = playerQuestData
								local alert = {
			                        text                     = "Task complete: "..questData.objectives[questData.currentObjective].objectiveName;
			                        textColor3                 = Color3.fromRGB(0, 0, 0);
			                        textStrokeTransparency     = 1;
			                        backgroundColor3        = Color3.fromRGB(255, 255, 162);
			                        backgroundTransparency    = 0;
									id = questData.id .. "s";
								                    }
								if playerStepData.hideAlert == nil then
			                    	--network:fireClient("alertPlayerNotification", player, alert, 6)
								end
							end
						end
					end
		...
		elseif triggerType == "item-collected" or triggerType == "item-lost" then
			...
			for i, playerQuestData in pairs(playerData.quests.active) do
				...
				for ii, playerObjectiveData in pairs(playerQuestData.objectives) do
					...
					if not playerObjectiveData.started then break end -- if you haven't talked through the next dialogue yet
					for iii, playerStepData in pairs(playerObjectiveData.steps) do
						if playerStepData.triggerType == triggerType and (playerStepData.requirement.id == triggerData.id) and playerStepData.completion.amount < playerStepData.requirement.amount then
							
							playerStepData.completion.amount = getItemCountByItemId(player, playerStepData.requirement.id) -- this fired twice when picked up rat head from ground
							...
							local itemName = itemLookup[triggerData.id] and itemLookup[triggerData.id].name or "Unknown"
							if playerStepData.hideAlert == nil then
								network:fireClient("alertPlayerNotification", player, {id = playerQuestData.id .. "s" .. itemName;
								 text = (itemName) .. ": " .. math.clamp(playerStepData.completion.amount,0, playerStepData.requirement.amount) .. " / " .. playerStepData.requirement.amount})
							end
							if playerStepData.completion.amount >= playerStepData.requirement.amount then
								local questData = playerQuestData
								local alert = {
			...
			for i, playerQuestData in pairs(playerData.quests.active) do
				...
				for ii, playerObjectiveData in pairs(playerQuestData.objectives) do
					...
					if not playerObjectiveData.started then break end -- if you haven't talked through the next dialogue yet
					for iii, playerStepData in pairs(playerObjectiveData.steps) do
						if playerStepData.triggerType == triggerType and  playerStepData.completion.amount < playerStepData.requirement.amount then
							local isTriggerValid = true
							for iiii, triggerCondition in pairs(playerStepData.requirement) do
								if iiii ~= "amount" and triggerCondition ~= triggerData[iiii] then
									isTriggerValid = false
								end
							end
							if isTriggerValid then
								playerStepData.completion.amount = playerStepData.completion.amount + 1
								if not modifiedQuest then
									table.insert(questsModified, playerQuestData)
									modifiedQuest = true
								...
								if playerStepData.requirement.amount > 1 then
									if triggerData.objectiveTitle then
										if playerStepData.hideAlert == nil then
											network:fireClient("alertPlayerNotification", player, {id = playerQuestData.id .. "s" .. triggerData.objectiveTitle;
											 text = (triggerData.objectiveTitle) .. ": " .. math.clamp(playerStepData.completion.amount,0, playerStepData.requirement.amount) .. " / " .. playerStepData.requirement.amount})
										end								
									end
								else
									if triggerData.objectiveTitle then
										if playerStepData.hideAlert == nil then
											network:fireClient("alertPlayerNotification", player, {id = playerQuestData.id .. "s" .. triggerData.objectiveTitle;
											 text = (triggerData.objectiveTitle) .. ": " .. "Completed"})	
										end							
									end
								end
								if playerStepData.completion.amount >= playerStepData.requirement.amount then
			...
			--[[
			warn("unrecognized quest trigger: `" .. tostring(triggerType) .. "`")
			return false
			]]
		end
		
		if #questsModified > 0 then
			for i, questData in pairs(questsModified) do
				-- quest completed!
				
				-- check to see if can move onto the next quest
				local currentObjective = questData.currentObjective
				
				if getPlayerQuestState(player, questData.id) == mapping.questState.objectiveDone or getPlayerQuestState(player, questData.id) == mapping.questState.handing then
					
					
					--questData.currentObjective = questData.currentObjective + 1  don't increment this here
					local questInfo = questLookup[questData.id]
					local alert = {
                        text                     = "Quest complete: "..questData.objectives[currentObjective].objectiveName.."! "..questData.objectives[currentObjective].completedText;
                        textColor3                 = Color3.fromRGB(0, 0, 0);
                        textStrokeTransparency     = 1;
                        backgroundColor3        = Color3.fromRGB(0, 255, 162);
                        backgroundTransparency    = 0;
						id = questData.id .. "s";
					                    }
					if questData.objectives[currentObjective].hideAlert == nil then
						 network:fireClient("alertPlayerNotification", player, alert, 6, "questComplete")
					end
                   
                    -- semi-tutorial for new players
...
local function int__createPlayerQuestDataFromQuestData(player, questData)
	local playerData = playerDataContainer[player]
	...
	local playerQuestData = {}
		playerQuestData.id 			= questData.id
		playerQuestData.objectives 	= {}
		playerQuestData.currentObjective = 1
		playerQuestData.handerName = questData.handerName or ""
		playerQuestData.QUEST_VERSION = questInfo.QUEST_VERSION or 1
	
	-- todo: population based on triggerType
	for i, objectiveData in pairs(questData.objectives) do
		local playerObjectiveData = {}
		playerObjectiveData.steps = {}
		playerObjectiveData.completedText = objectiveData.completedText
		playerObjectiveData.started = false
		playerObjectiveData.objectiveName = objectiveData.objectiveName
		
		if objectiveData.hideAlert then
			playerObjectiveData.hideAlert = true
		end
		-- new
		for ii, stepData in pairs(objectiveData.steps) do
			local playerStepData = {}
			playerStepData.triggerType = stepData.triggerType
			playerStepData.requirement = {}
			playerStepData.completion = {}
			playerStepData.completion.amount = 0
			if stepData.hideAlert then
			
				playerStepData.hideAlert = true
			end
			
			for  iii, v in pairs(stepData.requirement) do
				playerStepData.requirement[iii] = v
			end
			
			if playerStepData.triggerType == "item-collected" and playerStepData.requirement.id then
				warn("scanning for items already owned")
				for iii, inventorySlotData in pairs(playerData.inventory) do
					if inventorySlotData.id == playerStepData.requirement.id then
						warn("+1")
						playerStepData.completion.amount = playerStepData.completion.amount + (inventorySlotData.stacks or 1)
					end
				end
			end
				
			table.insert(playerObjectiveData.steps, playerStepData)
		end

		--warn("creating for",objectiveData.triggerType)
		table.insert(playerQuestData.objectives, playerObjectiveData)
	end
...
local function grantQuestToPlayer(player, questId, npcName)
	local playerData = playerDataContainer[player]
	
	for i, quest in pairs(playerData.quests.completed) do
		if quest.id == questId then
			if questLookup[questId].repeatableData ~= nil and questLookup[questId].repeatableData.value then
	...
	if playerData and questId and type(questId) == "number" then
		-- todo, support cooldown quests
		local playerQuestState = getPlayerQuestState(player, questId)
		local playerQuestCurrentObjective = getPlayerQuestCurrentObjective(player, questId) or 1
		
		if playerQuestState == mapping.questState.unassigned or playerQuestState == mapping.questState.completed then -- it can now be completed, some quests can be repeatable

			local questData = questLookup[questId]
			if questData then
				-- Now there are 2 different reasons the quest state would be unassigned: the player has not started the questline at all, or the questline already exists in their data but they are in between quests in the line
				-- requirements
				...
				if npcName then
					local questNpcParts = collectionService:GetTagged("interact")
					for i, part in pairs(questNpcParts) do
						if part.Parent and part.Parent.Name == npcName then
							if (player.Character.PrimaryPart.Position - part.Position).magnitude > 25 then
								return false
							end
						else 
							foundNPC = true
				...
				if npcName == nil then -- half to give a name for validation
					return false
				end
				
				-- level requirement
				if playerData.level < questData.objectives[playerQuestCurrentObjective].requireLevel then
					return false
				end
				
				-- quest requires player to complete other quests first
				for i, questId in pairs(questData.requireQuests) do
					local foundQuest = false
					for ii, savedQuestData in pairs(playerData.quests.completed) do
						if savedQuestData.id == questId then
							foundQuest = true
						end
					end
				...
				-- requires a certain class
				if questData.requireClass and not isPlayerOfClass(player, questData.requireClass) then
					return false
				end
				-- is the quest repeatable?
					for i, activeQuest in pairs(playerData.quests.active) do
						if activeQuest.id == questId and activeQuest.canStartAfterTime then
							if not questData.repeatableData.value then return end
							if os.time() < activeQuest.canStartAfterTime then return end
						end
					end

				local didFindQuestData = false
				for i, playerQuestData in pairs(playerData.quests.active) do
					if playerQuestData.id == questId then
						didFindQuestData = true
					end
				end
				
				
				if didFindQuestData then -- need to edit the existing quest data
					for i, playerQuestData in pairs(playerData.quests.active) do
						if playerQuestData.id == questId then
							
							local alert = {
			                    text                     = "Quest started: "..playerQuestData.objectives[playerQuestData.currentObjective].objectiveName;
			                    textColor3                 = Color3.fromRGB(0, 0, 0);
			                    textStrokeTransparency     = 1;
			                    backgroundColor3        = Color3.fromRGB(172,172,172);
			                    backgroundTransparency    = 0;
			                }
			                network:fireClient("alertPlayerNotification", player, alert, 4)

							
							if playerQuestData.currentObjective - 1 > 0 then
								local passed = true
								for ii, playerStepData in pairs(playerQuestData.objectives[playerQuestData.currentObjective - 1].steps) do
									if playerStepData.completion.amount < playerStepData.requirement.amount then
										passed = false
									end
								end
								if passed then
									playerQuestData.objectives[playerQuestData.currentObjective].started = true
									
									for v, playerStepData in pairs(playerQuestData.objectives[playerQuestData.currentObjective].steps) do
										if playerQuestData.currentObjective > 1 and playerStepData.triggerType == "item-collected" and playerStepData.requirement.id then
											warn("scanning for items already owned")
											for iii, inventorySlotData in pairs(playerData.inventory) do
												if inventorySlotData.id == playerStepData.requirement.id then
													warn("+1")
									...
									return true, mapping.questState.accepted
								end
							elseif playerQuestData.currentObjective == 1 then
								playerQuestData.objectives[playerQuestData.currentObjective].started = true
				...
				else -- need to create quest data
					for i, quest in pairs(playerData.quests.active) do
						if quest.id == questId then
							return false
						end
					...
					local playerQuestData = int__createPlayerQuestDataFromQuestData(player, questData)
				
					if playerQuestData then
						table.insert(playerData.quests.active, playerQuestData)
						
						playerQuestData.objectives[playerQuestData.currentObjective].started = true -- the first objective
						...
						return false, mapping.questState.denied
					end
				end
			end
		end
...
local function getPlayerInventoryTransfer_intermediateCollectionForQuest(player, questId)
	local questData = questLookup[questId]
	local playerData = playerDataContainer[player]
	local currentObjective = getPlayerQuestCurrentObjective(player, questId)
	if questData and playerData then
		if questData.objectives then
			local playerInventoryTransfer_intermediateCollection = {}
			--for i, objective in pairs(questData.objectives) do
				
				
				--if not questData.objectives[currentObjective].started then return {}, false end 
				
				for ii, step in pairs(questData.objectives[currentObjective].steps) do
					if step.triggerType == "item-collected" then
						
						
						local wasFound = false
						for iii, inventorySlotData in pairs(playerData.inventory) do
							if inventorySlotData.id == step.requirement.id and (inventorySlotData.stacks or 1) >= step.requirement.amount then
								table.insert(playerInventoryTransfer_intermediateCollection, {
									id 				= inventorySlotData.id;
									stacks 			= step.requirement.amount or 1;
									position 		= inventorySlotData.position;
									modifierData 	= inventorySlotData.modifierData;
								})
			...
			warn("forquest", #playerInventoryTransfer_intermediateCollection)
			
			return playerInventoryTransfer_intermediateCollection, true
		end
	end
...
local function int__convertQuestRewardsTableToPlayerInventoryTransfer_intermediateCollection(questRewards)
	if questRewards then
		local playerInventoryTransfer_intermediateCollection = {}
		
		for i, questReward in pairs(questRewards) do
			local itemBaseData = itemLookup[questReward.id]
			
			if itemBaseData then
				-- note: position will get auto-assigned by an inventory refresh
				table.insert(playerInventoryTransfer_intermediateCollection, {
					id 				= questReward.id;
					modifierData 	= questReward.modifierData;
					stacks 			= questReward.stacks;
				})
			end
		end
		
		return playerInventoryTransfer_intermediateCollection
	end
...
local function failQuest(player, questId)
	local playerData = playerDataContainer[player]
	if playerData and questId and type(questId) == "number" then
		...
		for i, playerQuestData in pairs(playerData.quests.active) do
			if playerQuestData.id == questId then
				...
				local questInfo = questLookup[questData.id]
				local alert = {
                    text                     = "Quest Failed: "..questData.questLineName;
                    textColor3                 = Color3.fromRGB(0, 0, 0);
                    textStrokeTransparency     = 1;
                    backgroundColor3        = Color3.fromRGB(255, 88, 66);
                    backgroundTransparency    = 0;
					id = questData.id .. "s";
			   }
				
				
				local addCompleted = true
				
				for ii, completedQuest in pairs(playerData.quests.completed) do
				...
				if addCompleted then
					table.insert(playerData.quests.completed, {
						id 				= questId;
						completionTime 	= tick(); 
						completionTimeUTC = os.time();
						failed			= true;
					})
				end
				
				
				network:fireClient("alertPlayerNotification", player, alert, 6, "questFailed")
...
local function submitQuest(player, questId, npcName)
	local playerData = playerDataContainer[player]
	
	if playerData and questId and type(questId) == "number" then
	
	local questData = questLookup[questId]
		-- this currently breaks if a quest is revised and an objective is added
		
		local isProblematicOldSave = false
		
		-- hacky fix if a new objective is added to a questline and a player had that quest active. stop infinite rewards
		for i, quest in pairs(playerData.quests.active) do
			if quest.id == questId then
				
				if #quest.objectives ~= #questData.objectives or (quest.currentObjective and quest.currentObjective > #questData.objectives) then
					table.remove(playerData.quests.active, i)
					
					
					
					local alert = {
						                        text                     = "Could not turn in quest: out of date quest version";
						                        textColor3                 = Color3.fromRGB(0, 0, 0);
						                        textStrokeTransparency     = 1;
						                        backgroundColor3        = Color3.fromRGB(255, 88, 66);
						                        backgroundTransparency    = 0;
		...
		local playerQuestState = getPlayerQuestState(player, questId)
		if playerQuestState == mapping.questState.objectiveDone or playerQuestState == mapping.questState.handing then
		
			local currentObjective = getPlayerQuestCurrentObjective(player, questId)

			if checkIfQuestIsCompleteByQuestId(player, questId, currentObjective) then	
				-- make sure to give the rewards after this point
				local finishedEntireQuest = false
				local saveCurrentObjective
				for i, playerQuestData in pairs(playerData.quests.active) do
					if playerQuestData.id == questId then
						...
						if questData.objectives[currentObjective].rewards then
							local rewards   = questData.objectives[currentObjective].rewards
							local goldMulti = questData.objectives[currentObjective].goldMulti
							local expMulti = questData.objectives[currentObjective].expMulti
							local level = questData.objectives[currentObjective].level
							
							local quest_playerInventoryTransfer_intermediateCollection = int__convertQuestRewardsTableToPlayerInventoryTransfer_intermediateCollection(rewards)
							local player_playerInventoryTransfer_intermediateCollection_quest, playerHasEverything = getPlayerInventoryTransfer_intermediateCollectionForQuest(player, questId)
							
							--PUT THIS BEFORE REWARDING ITEM!!!!!
							local foundNPC = false
							if npcName then
								local questNpcParts = collectionService:GetTagged("interact")
								for i, part in pairs(questNpcParts) do
							...
							if not foundNPC then return false end
							if npcName == nil then -- half to give a name for validation
								return false
							end
							...
							if playerHasEverything then
								local source = "npc:quest"
								local success, reason = int__tradeItemsBetweenPlayerAndNPC(player, player_playerInventoryTransfer_intermediateCollection_quest, 0, quest_playerInventoryTransfer_intermediateCollection,  levels.getQuestGoldFromLevel(level or 1) * (goldMulti or 1), source)
								...
								if success then
									...
									local alert = {
						                        text                     = "Could not accept reward: not enough inventory space.";
						                        textColor3                 = Color3.fromRGB(0, 0, 0);
						                        textStrokeTransparency     = 1;
						                        backgroundColor3        = Color3.fromRGB(255, 88, 66);
						                        backgroundTransparency    = 0;
								...
								local alert = {
						                        text                     = "Could not accept reward: you lack the required items.";
						                        textColor3                 = Color3.fromRGB(0, 0, 0);
						                        textStrokeTransparency     = 1;
						                        backgroundColor3        = Color3.fromRGB(255, 88, 66);
						                        backgroundTransparency    = 0;
						                    ...
						                    network:fireClient("alertPlayerNotification", player, alert, 6)
								return false, "does not own all needed items"
							end
						end
						
						if questData.objectives[currentObjective].serverOnFinish then
							spawn(function()
								questData.objectives[currentObjective].serverOnFinish(player)
							end)
						end
						...
						local expReward = levels.getQuestEXPFromLevel(level or 1) * (expMulti or 1)
						playerData.nonSerializeData.incrementPlayerData("exp", expReward)
						local expResults= {}
						expResults[player.Name] = expReward
						events:fireEventAll("playersXpGained", expResults)
						
						-- make sure only increment if not on last step or it wont give the reward
						local incremented = false -- can only complete one objective at a time silly
						if playerQuestData.currentObjective < #questData.objectives then
							playerQuestData.currentObjective = playerQuestData.currentObjective + 1
						else
							finishedEntireQuest = true
						end
						saveCurrentObjective = playerQuestData.currentObjective
					end
				end	
				...
				for i, playerQuestData in pairs(playerData.quests.active) do
					if playerQuestData.id == questId then
						...
						if finishedEntireQuest then
							table.remove(playerData.quests.active, i)
						
						
							
							if questData.repeatableData.value then
								local repeatQuest = int__createPlayerQuestDataFromQuestData(player, questData)
								--playerData.quests.active[i].objectives[1].started = true
								
								-- this is how quest repeating works, once you complete a quest that is repeatable it is returned to your active quests but with a timestamp flag that must be surpassed by os.time()
								repeatQuest.canStartAfterTime = os.time() + questData.repeatableData.timeInterval
								
								table.insert(playerData.quests.active, repeatQuest)
								
								-- in case a repeatable quest should act as a quest requirement, keep one copy of it in completed quests
								...
								if shouldAdd then
									table.insert(playerData.quests.completed, {
										id 				= questId;
										completionTime 	= tick(); -- spencer note: i'll leave this here just in case it's needed by something
										completionTimeUTC = os.time();
									})
								end
							else
							...
							if questId == 1 then
								spawn(function()
									game.BadgeService:AwardBadge(player.userId, 2124445667)
								end)
							end
						end
					end
...
local function changePlayerClass(player, class)
	...
	if playerData then
		-- check if first class change

		if playerData.class == "Adventurer" and playerData.level >= 10 and (class == "Hunter" or class == "Mage" or class == "Warrior") then
			local success, reason = grantPlayerAbilityBook(player, class)
			
			if success then
				playerData.nonSerializeData.setPlayerData("class", class)
				
				-- grant starter weapon
				local rewards = {}

				if class == "Hunter" then
					table.insert(rewards, {id = 15})
					table.insert(rewards, {id = 57; stacks = 1;})
					network:fireAllClients("signal_alertChatMessage", {
						Text = player.Name .. " has joined the ranks of the Hunters! Huzzah!"; 
						Font = Enum.Font.SourceSansBold; 
						Color = Color3.fromRGB(12, 255, 0)
					}) 						
				elseif class == "Mage" then
					table.insert(rewards, {id = 21})
					table.insert(rewards, {id = 58; stacks = 1;})
					network:fireAllClients("signal_alertChatMessage", {
						Text = player.Name .. " has been welcomed into the order of the Mages."; 
						Font = Enum.Font.SourceSansBold; 
						Color = Color3.fromRGB(0, 59, 255)
					}) 						
				elseif class == "Warrior" then
					table.insert(rewards, {id = 19})
					table.insert(rewards, {id = 56; stacks = 1;})
					network:fireAllClients("signal_alertChatMessage", {
						Text = player.Name .. " has pledged themselves to the the Warriors!"; 
						Font = Enum.Font.SourceSansBold; 
						Color = Color3.fromRGB(255, 0, 4)
					}) 					
				end
				
				if not playerData.flags.firstClassReward then
					playerData.flags.firstClassReward = true
					
					local wasAwardedItem = int__tradeItemsBetweenPlayerAndNPC(player, {}, 0, rewards, 0, "etc:treasure")
				end
			
				return true, "successfully changed class"
			else
				return success, reason
			end
		
		elseif playerData.class == "Hunter" and playerData.level >= 30 and (class == "Assassin" or class == "Ranger" or class == "Trickster") then
			if game.GameId == 712031239 then
				return false
			end
			local success, reason = grantPlayerAbilityBook(player, class)
		...
		elseif playerData.class == "Mage" and playerData.level >= 30 and (class == "Sorcerer" or class == "Warlock" or class == "Cleric") then
			if game.GameId == 712031239 then
				return false
			end
			local success, reason = grantPlayerAbilityBook(player, class)
		...
		elseif playerData.class == "Warrior" and playerData.level >= 30 and (class == "Knight" or class == "Paladin" or class == "Berserker") then
			if game.GameId == 712031239 then
				return false
			end
			local success, reason = grantPlayerAbilityBook(player, class)
			...
			return false, "unsupported class change attempt"
		end
	end
	
	return false, "invalid playerData"
end
network:create("changePlayerClass", "BindableFunction", "OnInvoke", changePlayerClass)
network:create("playerRequest_changePlayerClass", "RemoteFunction", "OnServerInvoke", changePlayerClass)
...
local function rebirthPlayer(player)
	if not isPlayerNearResetCharacter(player) then
		return false, "notNearResetCharacter"
	end
	...
	if not int__doesPlayerHaveInventorySpaceForTrade(player, {}, playerData.equipment) then
		return false, "notEnoughSpace"
	end
	
	-- remove all ability books except adventurer
	playerData.nonSerializeData.setPlayerData("abilityBooks", {
		adventurer = {
			pointsAssigned = 0,
		},
	})
	...
	-- set class back
	playerData.nonSerializeData.setPlayerData("class", "Adventurer")
	
	-- set level back
	playerData.nonSerializeData.setPlayerData("level", 10)
	...
	stats.int = 0
	stats.dex = 0
	playerData.nonSerializeData.setPlayerData("statistics", stats)
	
	-- remove all abilities
	playerData.nonSerializeData.setPlayerData("abilities", {})
	
	-- reset hotbar
	playerData.nonSerializeData.setPlayerData("hotbar", {})
	
	-- remove all equipment
	int__tradeItemsBetweenPlayerAndNPC(player, {}, 0, playerData.equipment, 0)
	playerData.nonSerializeData.setPlayerData("equipment", {})
	
	return true, ""
end
network:create("playerRequest_rebirthPlayer", "RemoteFunction", "OnServerInvoke", rebirthPlayer)
...
network:create("playerFailedQuest", "BindableEvent", "Event", failQuest)
...
-- Gonna dump my bounty stuff here -ber
...
local function playerRequest_primeArrow(player, category, inventorySlotPosition)
