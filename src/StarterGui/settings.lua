...
function module.init(Modules)
	...
	local function openPage(pageName)
		...
		for i,pageButton in pairs(script.Parent.header.buttons:GetChildren()) do
			if pageButton:IsA("ImageButton") then
				pageButton.ImageColor3 = Color3.fromRGB(212, 212, 212)
			end
		end
		local targetPage = script.Parent.pages:FindFirstChild(pageName)
		local targetPageButton = script.Parent.header.buttons:FindFirstChild(pageName)
	...
	-- volume slider
	local volumeFrame = script.Parent.pages.options.volume
	...
	local keybinds = script.Parent.pages.keybinds	
