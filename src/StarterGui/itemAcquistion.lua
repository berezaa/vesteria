...
local itemHoverFrame 	= script.Parent.contents
...
local BASIC_TWEEN_INFO 			= TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0)
...
function module.init(Modules)
	...
	local function reposition()
		...
		elseif Modules.input.mode.Value == "xbox" then
			...
			if frame then
				local x = frame.AbsolutePosition.X + frame.AbsoluteSize.X + 15
				if x + script.Parent.AbsoluteSize.X > screensize.X then
					x = frame.AbsolutePosition.X - script.Parent.AbsoluteSize.X - 15
				end 
	...
	local function populateItemHoverFrameWithTextData(textData)
		...
		script.Parent.Size = UDim2.new(0, 320 + 4, 0, 100)	
...
--		script.Parent.itemType.Visible = false
		cleanup()
		...
		script.Parent.UIScale.Scale = Modules.input.menuScale
		reposition()
	...
	local function populateItemHoverFrameWithAbility(abilityInfo, level, comparisonInfo, comparisonLevel)
		...
		script.Parent.Size = UDim2.new(0, 320 + 4, 0, 40 + 4)
		script.Parent.contents.Visible = false
		...
		if inputObject.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if hitPart then
				...
				if itemBaseData then
					...
					if targetPosition.Y.Offset + script.Parent.AbsoluteSize.Y > gameUI.AbsoluteSize.Y then
						targetPosition = UDim2.new(0, targetPosition.X.Offset, 0, gameUI.AbsoluteSize.Y - script.Parent.AbsoluteSize.Y)
					end
					
					-- clamp X axis to right side of the screen
					if targetPosition.X.Offset + script.Parent.AbsoluteSize.X > gameUI.AbsoluteSize.X then
						targetPosition = UDim2.new(0, gameUI.AbsoluteSize.X - script.Parent.AbsoluteSize.X, 0, targetPosition.Y.Offset)
					end
					
					script.Parent.Position = targetPosition
					]]
				else
					if script.Parent.Visible and module.source == "pickup" then
						script.Parent.Visible = false
					end
				end
			else
